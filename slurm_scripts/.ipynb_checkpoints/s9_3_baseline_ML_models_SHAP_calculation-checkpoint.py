...
def calculate_shap_values(X_train,X_test,model,model_name,shap_val_type=None):
    import shap
    ...
    if model_name in ['RandomForest','GradientBoost','XGBoost']:
        ...
        if shap_val_type=='interventional':
            ## Don't consider correlation between inputs variables ==> put weight only on variables that are being used by the model
            # https://arxiv.org/pdf/2006.16234
            explainer=shap.TreeExplainer(model,data=X_train)

        if shap_val_type=='observational':
            ## Consider correlation between inputs variables ==> put weight on all correlated variables, on those as well that are not being used by the model
        ...
        if shap_val_type=='observational':
            ## Cosider correlation between inputs variables ==> put weight on all correlated variables, on those as well that are not being used by the model
            ...
            for n, model_name in enumerate(model_names_[:]):
                ...
                for cv_repeat_num,_ in enumerate(range(len([*cv_results]))):
                    ...
                    for X_data,y_data,prefix in zip(X_list,
                        ...
                        for shap_val_type in ['observational','interventional']:
                        
                            shap_values=calculate_shap_values(X_train,X_data,model,model_name,shap_val_type)
                            ...
                            cv_results[f'cv_rep_{cv_repeat_num}'][f'{prefix}_shap_values_{shap_val_type}']=shap_values
