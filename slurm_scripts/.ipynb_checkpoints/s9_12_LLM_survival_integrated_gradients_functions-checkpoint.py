...
####=================================================
class SklearnCoxnetWrapper(nn.Module):
    def __init__(self, cox_model, alpha=None, dtype=torch.float32):
        """
        Wrap a fitted sksurv.linear_model.CoxnetSurvivalAnalysis model as a torch module.

        Parameters
        ----------
        cox_model : fitted CoxnetSurvivalAnalysis
            A trained scikit-survival CoxnetSurvivalAnalysis model.
        alpha : float or None
            If provided, select the coefficient vector corresponding to this alpha.
            If None, tries cox_model.alpha_ if available; otherwise falls back to
            the middle column if coef_ is 2D.
        dtype : torch.dtype
            Torch dtype for stored coefficients.
        """
        super().__init__()

        coef_matrix = cox_model.coef_

        if coef_matrix.ndim == 2:
            # shape: (n_features, n_alphas)
            if alpha is not None:
                best_idx = np.argmin(np.abs(cox_model.alphas_ - alpha))
            elif hasattr(cox_model, "alpha_"):
                best_idx = np.argmin(np.abs(cox_model.alphas_ - cox_model.alpha_))
            else:
                best_idx = coef_matrix.shape[1] // 2

            coef = coef_matrix[:, best_idx].ravel()
            self.selected_alpha = (
                float(cox_model.alphas_[best_idx]) if hasattr(cox_model, "alphas_") else None
            )
    ...
    def forward(self, x):
        ...
        risk_score : torch.Tensor
            Shape (batch,)
            Linear predictor / log-risk score for Cox model.
        """
        risk_score = torch.matmul(x, self.weights)  # (batch,)
        return risk_score
