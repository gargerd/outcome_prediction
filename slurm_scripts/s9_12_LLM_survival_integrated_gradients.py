...
from s9_12_LLM_survival_integrated_gradients_functions import *
                            ...
                            for pool_method in pool_methods_:
                                ...
                                for ther_arm_dur in ther_arm_durations_:
                                    ...
                                    for model_name in model_names_:
                                        ...
                                        fn=os.path.join(survival_anal_dir_,
                                                            f"{data_param_key}_{llm_model_name_with_tag}_{model_name}_{period_end_day}_days_{training_data_type}_4096_{data_inclusion_type}_autoenc_{autoencoder_merged}_{pool_method}_{ther_arm_dur}_vars_training_results.pickle")
                                        with open(fn, 'rb') as handle:
                                        ...
                                        #for cv_repeat_num in range(1): 
                                            ...
                                            if isinstance(cv_results[f'cv_rep_{cv_repeat_num}']['model'],dict):
                                                ...
                                                for n in range(len(cv_results[f'cv_rep_{cv_repeat_num}']['model'].keys())):
                                                    ...
                                                    lr_torch = SklearnCoxnetWrapper(lr_model).to(device)
                                                    ...
                                                    for batch in tqdm(test_loader, desc=f"{data_inclusion_type}/{period_end_day}",total=len(test_loader)):
                                                        ...
                                                        #base_dir_ = os.path.join(survival_anal_dir_,'integrated_gradients')
                                                        ig_output_dir=f"../../data/survival_analysis/integrated_gradients/{data_param_key}_{model_id}_{period_end_day}_days_{data_inclusion_type}_{pool_method}_{ther_arm_dur}"
