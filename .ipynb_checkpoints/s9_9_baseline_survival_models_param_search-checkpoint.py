            ...
            for training_data_type in training_data_type_:
                 ...
                 for ther_arm_dur in ther_arm_durations_[:]:
                    ...
                    for cv_repeat_num in range(train_params['num_cv_repeats']):
                        ...
                        print(f'++++++++++++++++++ \n{data_param_key} - Model: {model_name} - Period: {period_end_day} days  {ther_arm_dur} cohort - data for training: {training_data_type} - CV:{cv_repeat_num+1}\n+++++++++++++++++')
                        
                        final_pat_ids_for_analysis[period_end_day][ther_arm_dur]={}

                        X_subset__ = X_subset_[X_subset_['therapy_arm_duration']==ther_arm_dur].drop(columns=['therapy_arm_duration'])
                        
    
                        ## Split into train-test data
                        X_train,X_test,y_train,y_test,_,_ = create_std_training_testing_data(X_subset__,
                                                                                             y,
                                                                                             pat_ids_,
                    ...
                    fn=os.path.join(survival_anal_dir_,
                                    f"{data_param_key}_{model_name}_{period_end_day}_days_{training_data_type}_{ther_arm_dur}_param_search_results.pickle")
